...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.AGENT_SHIELD_LIVE_TEST === "1" ||
    process.env.AGENT_SHIELD_LIVE_GATEWAY === "1";
  ...
  const restore: RestoreEntry[] = [
    { key: "AGENT_SHIELD_TEST_FAST", value: process.env.AGENT_SHIELD_TEST_FAST },
    ...
    { key: "AGENT_SHIELD_STATE_DIR", value: process.env.AGENT_SHIELD_STATE_DIR },
    { key: "AGENT_SHIELD_CONFIG_PATH", value: process.env.AGENT_SHIELD_CONFIG_PATH },
    { key: "AGENT_SHIELD_GATEWAY_PORT", value: process.env.AGENT_SHIELD_GATEWAY_PORT },
    { key: "AGENT_SHIELD_BRIDGE_ENABLED", value: process.env.AGENT_SHIELD_BRIDGE_ENABLED },
    { key: "AGENT_SHIELD_BRIDGE_HOST", value: process.env.AGENT_SHIELD_BRIDGE_HOST },
    { key: "AGENT_SHIELD_BRIDGE_PORT", value: process.env.AGENT_SHIELD_BRIDGE_PORT },
    { key: "AGENT_SHIELD_CANVAS_HOST_PORT", value: process.env.AGENT_SHIELD_CANVAS_HOST_PORT },
    { key: "AGENT_SHIELD_TEST_HOME", value: process.env.AGENT_SHIELD_TEST_HOME },
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "agent-shield-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AGENT_SHIELD_TEST_HOME = tempHome;
  process.env.AGENT_SHIELD_TEST_FAST = "1";

  // Ensure test runs never touch the developer's real config/state, even if they have overrides set.
  delete process.env.AGENT_SHIELD_CONFIG_PATH;
  // Prefer deriving state dir from HOME so nested tests that change HOME also isolate correctly.
  delete process.env.AGENT_SHIELD_STATE_DIR;
  // Prefer test-controlled ports over developer overrides (avoid port collisions across tests/workers).
  delete process.env.AGENT_SHIELD_GATEWAY_PORT;
  delete process.env.AGENT_SHIELD_BRIDGE_ENABLED;
  delete process.env.AGENT_SHIELD_BRIDGE_HOST;
  delete process.env.AGENT_SHIELD_BRIDGE_PORT;
  delete process.env.AGENT_SHIELD_CANVAS_HOST_PORT;
