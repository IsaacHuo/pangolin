...
class SemanticAnalyzer:
    ...
    def __init__(
        ...
        cfg = config or FirewallConfig()
        self._fallback_classifier = MockClassifier()
        if classifier is not None:
            self._classifier = classifier
        elif cfg.l2_enabled:
            self._classifier = LlmClassifier(cfg)
        else:
            self._classifier = self._fallback_classifier
    ...
    async def analyze(
        ...
        try:
            ...
            # If live LLM analysis degrades to "fail-open", run local heuristics
            # as a safety net to catch obvious jailbreak/injection prompts.
            if self._should_use_local_fallback(result):
                fallback = await self._fallback_classifier.classify(method, params, context)
                if fallback.is_injection:
                    fallback.reasoning = (
                        f"{fallback.reasoning} (fallback due to upstream L2 failure: "
                        f"{result.reasoning})"
                    )
                    logger.warning(
                        "L2 fail-open fallback triggered block path: method=%s conf=%.2f",
                        method,
                        fallback.confidence,
                    )
                    return fallback
    ...
    def _should_use_local_fallback(self, result: L2Result) -> bool:
        """Return True when LLM classification degraded and should be sanity-checked locally."""
        if isinstance(self._classifier, MockClassifier):
            return False
        if result.is_injection:
            return False
        reason = (result.reasoning or "").lower()
        return any(
            marker in reason
            for marker in (
                "fail-open",
