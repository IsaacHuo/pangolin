...
const REPO = "agent-shield/agent-shield";
const PER_LINE = 10;
...
const onlyLogins = (mapConfig.onlyLogins ?? []).map((login) => login.toLowerCase());
    ...
    if (!existing) {
      ...
      if (user) {
        ...
        entriesByKey.set(key, {
          key,
          login: user.login,
          display: pickDisplay(baseName, user.login, undefined),
...
if (onlyLogins.length > 0) {
  const allow = new Set(onlyLogins);
  entries = entries.filter((entry) => entry.login && allow.has(entry.login.toLowerCase()));
}
