...
import { parseFrontmatter, resolveAgentShieldMetadata, resolveSkillInvocationPolicy, } from "./frontmatter.js";
...
function loadSkillEntries(workspaceDir, opts) {
    ...
    const extraSkills = mergedExtraDirs.flatMap((dir) => {
        ...
        return loadSkills({
            dir: resolved,
            source: "agent-shield-extra",
        });
    });
    const managedSkills = loadSkills({
        dir: managedSkillsDir,
        source: "agent-shield-managed",
    });
    ...
    const skillEntries = Array.from(merged.values()).map((skill) => {
        ...
        return {
            skill,
            frontmatter,
            metadata: resolveAgentShieldMetadata(frontmatter),
