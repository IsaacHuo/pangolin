...
import { DEFAULT_BROWSER_EVALUATE_ENABLED, DEFAULT_AGENT_SHIELD_BROWSER_COLOR, DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME, } from "../../browser/constants.js";
...
import { buildSandboxCreateArgs, dockerContainerState, execDocker, readDockerContainerLabel, readDockerPort, } from "./docker.js";
...
function buildSandboxBrowserResolvedConfig(params) {
    ...
    return {
        ...
        color: DEFAULT_AGENT_SHIELD_BROWSER_COLOR,
        executablePath: undefined,
        headless: params.headless,
        noSandbox: false,
        attachOnly: true,
        defaultProfile: DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME,
        profiles: {
            [DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params) {
    ...
    if (hasContainer) {
        ...
        currentHash = await readDockerContainerLabel(containerName, "agent-shield.configHash");
            ...
            if (isHot) {
                const hint = (() => {
                    if (params.cfg.scope === "session") {
                        return `agent-shield sandbox recreate --browser --session ${params.scopeKey}`;
                    }
                    if (params.cfg.scope === "agent") {
                        const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
                        return `agent-shield sandbox recreate --browser --agent ${agentId}`;
                    }
                    return "agent-shield sandbox recreate --browser --all";
                })();
    ...
    if (!hasContainer) {
        ...
        const args = buildSandboxCreateArgs({
            ...
            labels: { "agent-shield.sandboxBrowser": "1" },
        ...
        args.push("-e", `AGENT_SHIELD_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
        args.push("-e", `AGENT_SHIELD_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
        args.push("-e", `AGENT_SHIELD_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
        args.push("-e", `AGENT_SHIELD_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
        args.push("-e", `AGENT_SHIELD_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    const existingProfile = existing
        ? resolveProfile(existing.bridge.state.resolved, DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME)
        : null;
