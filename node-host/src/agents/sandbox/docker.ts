...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "agent-shield.sandbox=1");
  args.push("--label", `agent-shield.sessionKey=${params.scopeKey}`);
  args.push("--label", `agent-shield.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `agent-shield.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "agent-shield.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`agent-shield sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`agent-shield sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("agent-shield sandbox recreate --all");
}
