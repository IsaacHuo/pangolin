...
function resolveCacheTraceConfig(params) {
    ...
    const envEnabled = parseBooleanValue(env.AGENT_SHIELD_CACHE_TRACE);
    const enabled = envEnabled ?? config?.enabled ?? false;
    const fileOverride = config?.filePath?.trim() || env.AGENT_SHIELD_CACHE_TRACE_FILE?.trim();
    const filePath = fileOverride
        ? resolveUserPath(fileOverride)
        : path.join(resolveStateDir(env), "logs", "cache-trace.jsonl");
    const includeMessages = parseBooleanValue(env.AGENT_SHIELD_CACHE_TRACE_MESSAGES) ?? config?.includeMessages;
