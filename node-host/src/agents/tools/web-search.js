...
const WebSearchSchema = Type.Object({
    ...
    freshness: Type.Optional(Type.String({
        description: "Filter results by discovery time. Brave supports 'pd', 'pw', 'pm', 'py', and date range 'YYYY-MM-DDtoYYYY-MM-DD'. Perplexity supports 'pd', 'pw', 'pm', and 'py'.",
    })),
});
...
function missingSearchKeyPayload(provider) {
    ...
    return {
        ...
        message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("agent-shield configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
        docs: "https://docs.agent-shield.ai/tools/web",
    };
}
...
async function runPerplexitySearch(params) {
    ...
    const res = await fetch(endpoint, {
        ...
        headers: {
            ...
            "HTTP-Referer": "https://agent-shield.ai",
            "X-Title": "AgentShield Web Search",
        },
