...
function buildMessagingSection(params) {
    ...
    return [
        ...
        "- Never use exec/curl for provider messaging; AgentShield handles all routing internally.",
...
function buildDocsSection(params) {
    ...
    return [
        "## Documentation",
        `AgentShield docs: ${docsPath}`,
        "Mirror: https://docs.agent-shield.ai",
        "Source: https://github.com/agent-shield/agent-shield",
        "Community: https://discord.com/invite/clawd",
        "Find new skills: https://clawhub.com",
        "For AgentShield behavior, commands, config, or architecture: consult local docs first.",
        "When diagnosing issues, run `agent-shield status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
        "",
    ];
}
export function buildAgentSystemPrompt(params) {
    const coreToolSummaries = {
        ...
        gateway: "Restart, apply config, or run updates on the running AgentShield process",
    ...
    if (promptMode === "none") {
        return "You are a personal assistant running inside AgentShield.";
    }
    const lines = [
        "You are a personal assistant running inside AgentShield.",
        "",
        "## Tooling",
        ...
        toolLines.length > 0
            ...
            : [
                ...
                "- browser: control AgentShield's dedicated browser",
                "- canvas: present/eval/snapshot the Canvas",
                "- nodes: list/describe/notify/camera/screen on paired nodes",
                "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
        ...
        "",
        ...safetySection,
        "## AgentShield CLI Quick Reference",
        "AgentShield is controlled via subcommands. Do not invent commands.",
        "To manage the Gateway daemon service (start/stop/restart):",
        "- agent-shield gateway status",
        "- agent-shield gateway start",
        "- agent-shield gateway stop",
        "- agent-shield gateway restart",
        "If unsure, ask the user to run `agent-shield help` (or `agent-shield gateway --help`) and paste the output.",
        "",
        ...skillsSection,
        ...memorySection,
        // Skip self-update for subagent/none modes
        hasGateway && !isMinimal ? "## AgentShield Self-Update" : "",
        hasGateway && !isMinimal
            ? [
                "Get Updates (self-update) is ONLY allowed when the user explicitly asks for it.",
                "Do not run config.apply or update.run unless the user explicitly requests an update or config change; if it's not explicit, ask first.",
                "Actions: config.get, config.schema, config.apply (validate + write full config, then restart), update.run (update deps or git, then restart).",
                "After restart, AgentShield pings the last active session automatically.",
            ].join("\n")
            : "",
        hasGateway && !isMinimal ? "" : "",
        "",
        ...
        "These user-editable files are loaded by AgentShield and included below in Project Context.",
        "",
