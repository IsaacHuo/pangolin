export const PROJECT_NAME = "agent-shield";
export const LEGACY_PROJECT_NAMES = [];
export const MANIFEST_KEY = PROJECT_NAME;
export const LEGACY_MANIFEST_KEYS = LEGACY_PROJECT_NAMES;
export const LEGACY_PLUGIN_MANIFEST_FILENAMES = [];
export const LEGACY_CANVAS_HANDLER_NAMES = [];
export const MACOS_APP_SOURCES_DIR = "apps/macos/Sources/AgentShield";
export const LEGACY_MACOS_APP_SOURCES_DIRS = [];
