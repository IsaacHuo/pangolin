import type { AgentShieldConfig } from "../../config/types.agent-shield.js";
...
import { loadAgentShieldPlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: AgentShieldConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadAgentShieldPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadAgentShieldPlugins() already cache
