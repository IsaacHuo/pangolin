...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/agent-shield/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (AgentShield)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.AGENT_SHIELD_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const siblingCli = path.join(execDir, "agent-shield");
  ...
  const binCli = path.join(cwd, "bin", "agent-shield");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.AGENT_SHIELD_TAILNET_DNS?.trim();
