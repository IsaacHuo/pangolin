...
export function resolveGatewayAuth(params) {
    ...
    const token = authConfig.token ?? env.AGENT_SHIELD_GATEWAY_TOKEN ?? undefined;
    const password = authConfig.password ?? env.AGENT_SHIELD_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth) {
    if (auth.mode === "token" && !auth.token) {
        ...
        throw new Error("gateway auth mode is token, but no token was configured (set gateway.auth.token or AGENT_SHIELD_GATEWAY_TOKEN)");
    }
