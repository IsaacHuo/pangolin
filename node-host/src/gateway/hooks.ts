...
export function resolveHooksConfig(cfg: AgentShieldConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AgentShieldConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-agent-shield-token"] === "string"
      ? req.headers["x-agent-shield-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
