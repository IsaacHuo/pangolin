...
export function buildGatewayConnectionDetails(
  options: { config?: AgentShieldConfig; url?: string; configPath?: string } = {},
...
export async function callGateway<T = Record<string, unknown>>(
    ...
    (!urlOverride
      ? isRemoteMode
        ? typeof remote?.token === "string" && remote.token.trim().length > 0
          ? remote.token.trim()
          : undefined
        : process.env.AGENT_SHIELD_GATEWAY_TOKEN?.trim() ||
  ...
  const password =
    ...
    (!urlOverride
      ? process.env.AGENT_SHIELD_GATEWAY_PASSWORD?.trim() ||
