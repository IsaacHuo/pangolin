...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.AGENT_SHIELD_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.AGENT_SHIELD_STATE_DIR;
  const configPath = env.AGENT_SHIELD_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    AGENT_SHIELD_PROFILE: profile,
    AGENT_SHIELD_STATE_DIR: stateDir,
    AGENT_SHIELD_CONFIG_PATH: configPath,
    AGENT_SHIELD_GATEWAY_PORT: String(port),
    AGENT_SHIELD_GATEWAY_TOKEN: token,
    AGENT_SHIELD_LAUNCHD_LABEL: resolvedLaunchdLabel,
    AGENT_SHIELD_SYSTEMD_UNIT: systemdUnit,
    AGENT_SHIELD_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    AGENT_SHIELD_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    AGENT_SHIELD_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    AGENT_SHIELD_STATE_DIR: stateDir,
    AGENT_SHIELD_CONFIG_PATH: configPath,
    AGENT_SHIELD_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    AGENT_SHIELD_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    AGENT_SHIELD_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    AGENT_SHIELD_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    AGENT_SHIELD_LOG_PREFIX: "node",
    AGENT_SHIELD_SERVICE_MARKER: NODE_SERVICE_MARKER,
    AGENT_SHIELD_SERVICE_KIND: NODE_SERVICE_KIND,
    AGENT_SHIELD_SERVICE_VERSION: VERSION,
  };
}
