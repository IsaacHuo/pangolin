...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.AGENT_SHIELD_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.AGENT_SHIELD_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.AGENT_SHIELD_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.AGENT_SHIELD_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
