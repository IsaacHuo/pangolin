...
const SAFE_DAEMON_ENV_KEYS = [
  "AGENT_SHIELD_PROFILE",
  "AGENT_SHIELD_STATE_DIR",
  "AGENT_SHIELD_CONFIG_PATH",
  "AGENT_SHIELD_GATEWAY_PORT",
  "AGENT_SHIELD_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("agent-shield gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.AGENT_SHIELD_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("agent-shield gateway install", env),
    formatCliCommand("agent-shield gateway", env),
  ];
