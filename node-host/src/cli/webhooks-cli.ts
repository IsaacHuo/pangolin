...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.agent-shield.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AgentShield hooks")
    ...
    .option("--hook-url <url>", "AgentShield hook URL")
    .option("--hook-token <token>", "AgentShield hook token")
