...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("agent-shield node install"), formatCliCommand("agent-shield node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("agent-shield node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AGENT_SHIELD_LOG_PREFIX: baseEnv.AGENT_SHIELD_LOG_PREFIX ?? "node",
