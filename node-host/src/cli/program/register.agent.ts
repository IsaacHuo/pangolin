...
${formatHelpExamples([
  ['agent-shield agent --to +15555550123 --message "status update"', "Start a new session."],
  ['agent-shield agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'agent-shield agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'agent-shield agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['agent-shield agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'agent-shield agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.agent-shield.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.agent-shield.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['agent-shield agents set-identity --agent main --name "AgentShield" --emoji "🦞"', "Set name + emoji."],
  ["agent-shield agents set-identity --agent main --avatar avatars/agent-shield.png", "Set avatar path."],
  [
    "agent-shield agents set-identity --workspace ~/.agent-shield/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "agent-shield agents set-identity --identity-file ~/.agent-shield/workspace/IDENTITY.md --agent main",
