...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: AgentShieldConfig; warnings: string[] } {
...
function enablePluginInConfig(config: AgentShieldConfig, pluginId: string): AgentShieldConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage AgentShield plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.agent-shield.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: AgentShieldConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.agent-shield.ai/plugin");
      lines.push("");
