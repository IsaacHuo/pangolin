...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "agent-shield";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "agent-shield"}`,
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (agent-shield|extension). Default: agent-shield")
    .action(
