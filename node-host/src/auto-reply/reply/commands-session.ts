...
import { scheduleGatewaySigusr1Restart, triggerAgentShieldRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AgentShield in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAgentShieldRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AgentShield via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
