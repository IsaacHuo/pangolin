...
function prefersSips() {
    return (process.env.AGENT_SHIELD_IMAGE_BACKEND === "sips" ||
        (process.env.AGENT_SHIELD_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin"));
}
...
async function withTempDir(fn) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agent-shield-img-"));
    try {
        return await fn(dir);
    }
    finally {
