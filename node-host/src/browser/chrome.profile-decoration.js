...
function decoratedMarkerPath(userDataDir) {
    return path.join(userDataDir, ".agent-shield-profile-decorated");
}
 ...
 */
export function decorateAgentShieldProfile(userDataDir, opts) {
    const desiredName = opts?.name ?? DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME;
    const desiredColor = (opts?.color ?? DEFAULT_AGENT_SHIELD_BROWSER_COLOR).toUpperCase();
