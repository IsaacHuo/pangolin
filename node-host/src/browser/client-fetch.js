...
function withLoopbackBrowserAuthImpl(url, init, deps) {
    ...
    if (headers.has("authorization") || headers.has("x-agent-shield-password")) {
        return { ...init, headers };
    }
    ...
    try {
        ...
        if (auth.password) {
            headers.set("x-agent-shield-password", auth.password);
            return { ...init, headers };
        }
    }
    catch {
    ...
    try {
        ...
        if (bridgeAuth?.token) {
            ...
            headers.set("x-agent-shield-password", bridgeAuth.password);
        }
    }
    catch {
        // ignore
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    const modelHint = "Do NOT retry the browser tool — it will keep failing. " +
    ...
    if (looksLikeTimeout) {
        return new Error(`Can't reach the AgentShield browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`);
    }
    return new Error(`Can't reach the AgentShield browser control service. ${operatorHint} ${modelHint} (${msg})`);
}
