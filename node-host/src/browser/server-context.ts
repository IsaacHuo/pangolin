...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchAgentShieldChrome,
  resolveAgentShieldUserDataDir,
  stopAgentShieldChrome,
} from "./chrome.js";
...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the AgentShield Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by agent-shield. ` +
