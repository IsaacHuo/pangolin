import type { BrowserConfig, BrowserProfileConfig, AgentShieldConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_AGENT_SHIELD_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "agent-shield" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_AGENT_SHIELD_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "agent-shield" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AGENT_SHIELD_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AgentShield browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for agent-shield/agent-shield CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AgentShieldConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AGENT_SHIELD_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "agent-shield";
