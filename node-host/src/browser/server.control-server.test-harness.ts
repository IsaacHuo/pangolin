...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/agent-shield" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "agent-shield-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "agent-shield",
        profiles: {
          "agent-shield": { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchAgentShieldChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveAgentShieldUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopAgentShieldChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.AGENT_SHIELD_GATEWAY_PORT;
    process.env.AGENT_SHIELD_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.AGENT_SHIELD_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.AGENT_SHIELD_GATEWAY_PASSWORD;
    delete process.env.AGENT_SHIELD_GATEWAY_TOKEN;
    delete process.env.AGENT_SHIELD_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.AGENT_SHIELD_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.AGENT_SHIELD_GATEWAY_TOKEN;
    } else {
      process.env.AGENT_SHIELD_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.AGENT_SHIELD_GATEWAY_PASSWORD;
    } else {
      process.env.AGENT_SHIELD_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
