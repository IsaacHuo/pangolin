...
import { isChromeCdpReady, isChromeReachable, launchAgentShieldChrome, resolveAgentShieldUserDataDir, stopAgentShieldChrome, } from "./chrome.js";
...
export function listKnownProfileNames(state) {
...
function createProfileContext(opts, profile) {
    ...
    const ensureBrowserAvailable = async () => {
        ...
        if (isExtension) {
            ...
            throw new Error(`Chrome extension relay is running, but no tab is connected. Click the AgentShield Chrome extension icon on a tab to attach it (profile "${profile.name}").`);
        }
        if (!httpReachable) {
            ...
            const launched = await launchAgentShieldChrome(current.resolved, profile);
        ...
        if (!profileState.running) {
            throw new Error(`Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by agent-shield. ` +
        ...
        await stopAgentShieldChrome(profileState.running);
        setProfileRunning(null);
        const relaunched = await launchAgentShieldChrome(current.resolved, profile);
        ...
        if (tabs1.length === 0) {
            if (profile.driver === "extension") {
                throw new Error(`tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
                    "Click the AgentShield Browser Relay toolbar icon on the tab you want to control (badge ON).");
            }
