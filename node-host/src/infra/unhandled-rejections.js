...
export function isUnhandledRejectionHandled(reason) {
    for (const handler of handlers) {
        try {
            ...
            console.error("[pangolin] Unhandled rejection handler failed:", err instanceof Error ? (err.stack ?? err.message) : err);
        }
    }
    return false;
}
export function installUnhandledRejectionHandler() {
    process.on("unhandledRejection", (reason, _promise) => {
        ...
        if (isTransientNetworkError(reason)) {
            console.warn("[pangolin] Non-fatal unhandled rejection (continuing):", formatUncaughtError(reason));
            return;
        }
