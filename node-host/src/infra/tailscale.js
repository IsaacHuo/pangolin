...
export async function getTailscaleBinary() {
    const forcedBinary = process.env.AGENT_SHIELD_TEST_TAILSCALE_BINARY?.trim();
    if (forcedBinary) {
...
export async function ensureFunnel(port, exec = runExec, runtime = defaultRuntime, prompt = promptYesNo) {
    ...
    try {
        ...
        runtime.error(info(`Tip: Funnel is optional for AgentShield. You can keep running the web gateway without it: \`${formatCliCommand("agent-shield gateway")}\``));
        if (shouldLogVerbose()) {
            const rich = isRich();
            if (stdout.trim()) {
