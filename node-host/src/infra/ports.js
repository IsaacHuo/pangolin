...
export async function handlePortError(err, port, context, runtime = defaultRuntime) {
    ...
    if (err instanceof PortInUseError || (isErrno(err) && err.code === "EADDRINUSE")) {
        ...
        if (details) {
            ...
            if (/agent-shield|src\/index\.ts|dist\/index\.js/.test(details)) {
                runtime.error(warn("It looks like another AgentShield instance is already running. Stop it or pick a different port."));
            }
        }
