...
const GATEWAY_SERVICE_TYPE = "_agent-shield-gw._tcp";
...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    const match = line.match(/_agent-shield-gw\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
async function discoverWideAreaViaTailnetDns(
  ...
  for (const ptr of ptrs) {
    ...
    const instanceName = ptrName.replace(/\.?_agent-shield-gw\._tcp\..*$/, "");
