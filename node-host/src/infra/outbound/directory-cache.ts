...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AgentShieldConfig | null = null;
  ...
  get(key: string, cfg: AgentShieldConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AgentShieldConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: AgentShieldConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: AgentShieldConfig): void {
