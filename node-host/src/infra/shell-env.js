...
export function loadShellEnvFallback(opts) {
    ...
    try {
        ...
        logger.warn(`[pangolin] shell env fallback failed: ${msg}`);
        lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
    return isTruthyEnvValue(env.AGENT_SHIELD_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
    return isTruthyEnvValue(env.AGENT_SHIELD_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
    const raw = env.AGENT_SHIELD_SHELL_ENV_TIMEOUT_MS?.trim();
    if (!raw) {
