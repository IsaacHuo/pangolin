...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.AGENT_SHIELD_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/agent-shield/dist/hooks/bundled-dir.js
  try {
