...
import type {
  AgentShieldPluginDefinition,
  AgentShieldPluginModule,
  PluginDiagnostic,
  PluginLogger,
} from "./types.js";
...
import { discoverAgentShieldPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AgentShieldPluginDefinition;
  register?: AgentShieldPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AgentShieldPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AgentShieldPluginDefinition;
...
export function loadAgentShieldPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAgentShieldPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "agent-shield/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "agent-shield/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AgentShieldPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as AgentShieldPluginModule;
    } catch (err) {
