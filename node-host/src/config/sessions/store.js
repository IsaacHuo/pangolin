...
function getSessionStoreTtl() {
    return resolveCacheTtlMs({
        envValue: process.env.AGENT_SHIELD_SESSION_CACHE_TTL_MS,
...
/**
 * Resolve maintenance settings from agent-shield.json (`session.maintenance`).
...
async function drainSessionStoreLockQueue(storePath) {
    ...
    try {
        while (queue.pending.length > 0) {
            ...
            task.started = true;
            const remainingTimeoutMs = task.timeoutAt != null
