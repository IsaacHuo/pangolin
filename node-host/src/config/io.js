...
import { compareAgentShieldVersions } from "./version.js";
...
function warnIfConfigFromFuture(cfg, logger) {
    ...
    const cmp = compareAgentShieldVersions(VERSION, touched);
    if (cmp === null) {
        return;
    }
    if (cmp < 0) {
        logger.warn(`Config was last written by a newer AgentShield (${touched}); current version is ${VERSION}.`);
    }
}
...
export function createConfigIO(overrides = {}) {
    ...
    async function writeConfigFile(cfg, options = {}) {
        ...
        const logConfigOverwrite = () => {
            ...
            const shouldLogInVitest = deps.env.AGENT_SHIELD_TEST_CONFIG_OVERWRITE_LOG === "1";
        ...
        const logConfigWriteAnomalies = () => {
            ...
            const isVitest = deps.env.VITEST === "true";
            const shouldLogInVitest = deps.env.AGENT_SHIELD_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
            if (isVitest && !shouldLogInVitest) {
                return;
            }
        ...
        const auditRecordBase = {
            ...
            watchMode: deps.env.AGENT_SHIELD_WATCH_MODE === "1",
            watchSession: typeof deps.env.AGENT_SHIELD_WATCH_SESSION === "string" &&
                deps.env.AGENT_SHIELD_WATCH_SESSION.trim().length > 0
                ? deps.env.AGENT_SHIELD_WATCH_SESSION.trim()
                : null,
            watchCommand: typeof deps.env.AGENT_SHIELD_WATCH_COMMAND === "string" &&
                deps.env.AGENT_SHIELD_WATCH_COMMAND.trim().length > 0
                ? deps.env.AGENT_SHIELD_WATCH_COMMAND.trim()
                : null,
            existsBefore: snapshot.exists,
...
// module scope. `AGENT_SHIELD_CONFIG_PATH` (and friends) are expected to work even
// when set after the module has been imported (tests, one-off scripts, etc.).
const DEFAULT_CONFIG_CACHE_MS = 200;
let configCache = null;
function resolveConfigCacheMs(env) {
    const raw = env.AGENT_SHIELD_CONFIG_CACHE_MS?.trim();
    if (raw === "" || raw === "0") {
        return 0;
    }
    if (!raw) {
...
function shouldUseConfigCache(env) {
    if (env.AGENT_SHIELD_DISABLE_CONFIG_CACHE?.trim()) {
        return false;
    }
