...
import { AgentShieldSchema } from "./zod-schema.js";

export type { ConfigUiHint, ConfigUiHints } from "./schema.hints.js";

export type ConfigSchema = ReturnType<typeof AgentShieldSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = AgentShieldSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "AgentShieldConfig";
  const hints = mapSensitivePaths(AgentShieldSchema, "", buildBaseHints());
  const next = {
