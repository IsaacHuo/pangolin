...
type ConsoleConfigLoader = () => AgentShieldConfig["logging"] | undefined;
const loadConfigFallbackDefault: ConsoleConfigLoader = () => {
  try {
    const loaded = requireConfig("../config/config.js") as {
      loadConfig?: () => AgentShieldConfig;
    };
...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && process.env.AGENT_SHIELD_TEST_CONSOLE !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  let cfg: AgentShieldConfig["logging"] | undefined =
