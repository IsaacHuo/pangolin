...
<title>AgentShield Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.agentShieldCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.agentShieldCanvasA2UIAction &&
        typeof window.agentShieldCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.agentShieldSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("agent-shield:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AgentShield node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.agentShieldSendUserAction === "function"
        ? window.agentShieldSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AGENT_SHIELD_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AGENT_SHIELD_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AgentShield Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
