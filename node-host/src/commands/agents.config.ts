...
type AgentEntry = NonNullable<NonNullable<AgentShieldConfig["agents"]>["list"]>[number];

export type AgentIdentity = AgentIdentityFile;

export function listAgentEntries(cfg: AgentShieldConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AgentShieldConfig, agentId: string) {
...
function resolveAgentModel(cfg: AgentShieldConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AgentShieldConfig): AgentSummary[] {
