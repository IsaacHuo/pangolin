...
export async function setupSkills(
  ...
  let next: AgentShieldConfig = cfg;
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("agent-shield doctor")}\` to review skills + requirements.`,
      );
      runtime.log("Docs: https://docs.agent-shield.ai/skills");
    }
  }
