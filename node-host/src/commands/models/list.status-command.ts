...
import { resolveAgentShieldAgentDir } from "../../agents/agent-paths.js";
import {
  resolveAgentDir,
...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAgentShieldAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("agent-shield models auth setup-token")}\` or \`${formatCliCommand("agent-shield configure")}\`.`
          : `Run \`${formatCliCommand("agent-shield configure")}\` or set an API key env var.`;
