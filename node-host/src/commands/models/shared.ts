...
export async function updateConfig(
  mutator: (cfg: AgentShieldConfig) => AgentShieldConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AgentShieldConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AgentShieldConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("agent-shield agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
