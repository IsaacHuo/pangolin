import type { AgentShieldConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: AgentShieldConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.AGENT_SHIELD_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.AGENT_SHIELD_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: AgentShieldConfig) {
