...
const SEARCH_TOOL = "https://docs.agent-shield.ai/mcp.SearchAgentShield";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.agent-shield.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('agent-shield docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.agent-shield.ai/");
      runtime.log(`Search: ${formatCliCommand('agent-shield docs "your query"')}`);
    }
    return;
  }
