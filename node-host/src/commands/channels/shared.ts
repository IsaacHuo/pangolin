...
import { type AgentShieldConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
...
): Promise<AgentShieldConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("agent-shield doctor")}.`);
    runtime.exit(1);
    return null;
  }
