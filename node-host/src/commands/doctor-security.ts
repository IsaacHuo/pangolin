...
import type { AgentShieldConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AgentShieldConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("agent-shield security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("agent-shield configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("agent-shield config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("agent-shield doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "agent-shield config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("agent-shield config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('agent-shield config set session.dmScope "per-channel-peer"') +
