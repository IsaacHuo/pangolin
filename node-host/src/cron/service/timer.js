...
import { computeJobNextRunAtMs, nextWakeAtMs, recomputeNextRuns, recomputeNextRunsForMaintenance, resolveJobPayloadTextForMain, } from "./jobs.js";
...
export async function onTimer(state) {
    if (state.running) {
        ...
        // execution to finish.
        // See: https://github.com/agent-shield/agent-shield/issues/12025
        if (state.timer) {
            clearTimeout(state.timer);
        }
        state.timer = setTimeout(() => {
...
async function executeJobCore(state, job) {
    ...
    // See: https://github.com/agent-shield/agent-shield/issues/15692
