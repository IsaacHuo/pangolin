        ...
        if (isDueOrMissing) {
            try {
                ...
                if (errorCount >= MAX_SCHEDULE_ERRORS) {
                    job.enabled = false;
                    state.deps.log.error({ jobId: job.id, name: job.name, errorCount, err: String(err) }, "cron: auto-disabled job after repeated schedule errors");
                }
                else {
                    state.deps.log.warn({ jobId: job.id, name: job.name, errorCount, err: String(err) }, "cron: failed to compute next run for job (skipping)");
                }
            }
        }
        return changed;
