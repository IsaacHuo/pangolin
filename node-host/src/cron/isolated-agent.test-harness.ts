...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "agent-shield-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".agent-shield", "sessions");
...
export function makeCfg(
  ...
  const base: AgentShieldConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "agent-shield"),
      },
    },
